...
class UserNetworkBuilder(GraphBuilder):
    ...
    def build_answer_network(self, limit: int = 100) -> Graph:
        ...
        self.graph.vs["user_id"] = [
            unique_users[i] if i < len(unique_users) else None
            for i in range(len(self.graph.vs))
        ]
        self.graph.vs["name"] = [
            f"User_{unique_users[i]}" if i < len(unique_users) else f"V{i}"
            for i in range(len(self.graph.vs))
        ]

        return self.graph

    def build_answer_network_with_reputation(
        self, limit: int = 100
    ) -> Tuple[Graph, pd.DataFrame]:
        """建構含聲望資訊的回答網路（研究方法1: 使用者聲望與網路中心度）"""
        posts_df = self.data_loader.load_posts_with_answers(
            limit=limit, only_accepted=False
        )
        ...
        unique_users = list(
            set([e[0] for e in edge_counts.keys()] + [e[1] for e in edge_counts.keys()])
        )
        ...
        self.graph.vs["reputation"] = [
            user_reputation.get(uid, 0) for uid in unique_users
        ]
        self.graph.vs["reputation_level"] = [
            user_reputation_level.get(uid, "0_None") for uid in unique_users
        ]
        ...
        self.graph.vs["centrality_level"] = [
            "high" if b >= median_bt else "low" for b in betweenness
        ]

        return self.graph, users_df[users_df["id"].isin(unique_users)]

    def build_connectivity_network(
        self, limit: int = 100
    ) -> Tuple[Graph, pd.DataFrame]:
        """
        建構用戶連通性網路
        ...
        connectivity_df = self.data_loader.load_user_connectivity(
            limit=limit * 2, user_ids=unique_users
        )
        connectivity_df_filtered = connectivity_df[
            connectivity_df["user_id"].isin(unique_users)
        ]
        ...
        self.graph.vs["name"] = [f"U{uid}" for uid in unique_users]
        self.graph.vs["reputation"] = [
            user_data.get(uid, {}).get("reputation", 0) for uid in unique_users
        ]
        self.graph.vs["total_interactions"] = [
            user_data.get(uid, {}).get("total_interactions", 0) for uid in unique_users
        ]
        self.graph.vs["connectivity_level"] = [
            user_data.get(uid, {}).get("connectivity_level", "0_None")
        ...
        self.graph.vs["interaction_type"] = [
            "continuous"
            if user_data.get(uid, {}).get("total_interactions", 0) > 1
            else "single"
            for uid in unique_users
        ]
    ...
    def build_account_age_network(self, limit: int = 100) -> Tuple[Graph, pd.DataFrame]:
        ...
        age_df = self.data_loader.load_account_age(
            limit=limit * 2, user_ids=unique_users
        )
        ...
        self.graph.vs["account_age_days"] = [
            user_data.get(uid, {}).get("account_age_days", 0) for uid in unique_users
        ]
        self.graph.vs["account_age_level"] = [
            user_data.get(uid, {}).get("account_age_level", "0_None")
            for uid in unique_users
        ]
        self.graph.vs["post_type"] = [
            user_data.get(uid, {}).get("post_type", "4_Neither") for uid in unique_users
        ]
        self.graph.vs["question_count"] = [
            int(user_data.get(uid, {}).get("question_count", 0)) for uid in unique_users
        ]
        self.graph.vs["answer_count"] = [
            int(user_data.get(uid, {}).get("answer_count", 0)) for uid in unique_users
        ]
...
class TagNetworkBuilder(GraphBuilder):
    ...
    def build_tag_cooccurrence_network(
        ...
        tag_popularity = dict(
            zip(popularity_df["tag_name"], popularity_df["usage_count"])
        )
        ...
        self.graph.vs["popularity"] = [
            tag_popularity.get(tag, 0) for tag in unique_tags
        ]
        self.graph.vs["tech_domain"] = [
            self._get_tag_domain(tag) for tag in unique_tags
        ]
...
class MultiInteractionNetworkBuilder(GraphBuilder):
    """多类型互动网络构建器 - 组合多种用户互动"""
    ...
    def build_combined_network(
        ...
        if include_answers:
            ...
            for _, row in posts_df.iterrows():
                if pd.notna(row["answerer_id"]) and pd.notna(row["questioner_id"]):
                    key = (int(row["answerer_id"]), int(row["questioner_id"]))
                    all_edges[key] = all_edges.get(key, 0) + 1
                    all_users.add(int(row["answerer_id"]))
                    all_users.add(int(row["questioner_id"]))

        if include_comments:
            try:
                sql = f"""
                SELECT c.user_id AS commenter_id, p.owner_user_id AS post_owner_id
                ...
                for _, row in comments_df.iterrows():
                    if pd.notna(row["commenter_id"]) and pd.notna(row["post_owner_id"]):
                        key = (int(row["commenter_id"]), int(row["post_owner_id"]))
                        all_edges[key] = all_edges.get(key, 0) + 1
                        all_users.add(int(row["commenter_id"]))
                        all_users.add(int(row["post_owner_id"]))
            except Exception:
                pass
        ...
        if include_votes:
            try:
                sql = f"""
                SELECT v.user_id AS voter_id, p.owner_user_id AS post_owner_id
                ...
                for _, row in votes_df.iterrows():
                    if pd.notna(row["voter_id"]) and pd.notna(row["post_owner_id"]):
                        key = (int(row["voter_id"]), int(row["post_owner_id"]))
                        all_edges[key] = all_edges.get(key, 0) + 1
                        all_users.add(int(row["voter_id"]))
                        all_users.add(int(row["post_owner_id"]))
            except Exception:
                pass
        ...
        if include_edits:
            try:
                sql = f"""
                SELECT e.user_id AS editor_id, p.owner_user_id AS post_owner_id
                FROM `bigquery-public-data.stackoverflow.posts_questions` p
                INNER JOIN (
                    SELECT post_id, user_id, ROW_NUMBER() OVER (PARTITION BY post_id ORDER BY creation_date) as rn
                ...
                for _, row in edits_df.iterrows():
                    if pd.notna(row["editor_id"]) and pd.notna(row["post_owner_id"]):
                        key = (int(row["editor_id"]), int(row["post_owner_id"]))
                        all_edges[key] = all_edges.get(key, 0) + 1
                        all_users.add(int(row["editor_id"]))
                        all_users.add(int(row["post_owner_id"]))
            except Exception:
                pass
        ...
        if not unique_users:
            ...
            return self.graph, pd.DataFrame()
        ...
        # 過濾掉度數<2的節點（只保留有足夠互動的用戶）
        # 需要迭代過濾，因為移除節點後可能產生新的孤立節點
        max_iterations = 10
        for _ in range(max_iterations):
            degrees = self.graph.degree()
            keep_indices = [i for i, d in enumerate(degrees) if d >= 2]
            if len(keep_indices) == len(degrees):
                break  # 所有節點都滿足條件
            if len(keep_indices) == 0:
                break  # 沒有節點滿足條件
            self.graph = self.graph.induced_subgraph(keep_indices)

        # 最後確保沒有度數=0的節點
        degrees = self.graph.degree()
        keep_indices = [i for i, d in enumerate(degrees) if d > 0]
        if len(keep_indices) > 0 and len(keep_indices) < len(degrees):
            self.graph = self.graph.induced_subgraph(keep_indices)
...
class CorePeripheryBuilder(GraphBuilder):
    ...
    def build_core_periphery_network(
        self, limit: int = 100
    ) -> Tuple[Graph, pd.DataFrame]:
        """
        建構核心-邊緣結構網路
        ...
        for _, row in posts_df.iterrows():
            if pd.notna(row["answerer_id"]) and pd.notna(row["questioner_id"]):
                key = (row["answerer_id"], row["questioner_id"])
                edge_counts[key] = edge_counts.get(key, 0) + 1
                answer_time_levels[row["questioner_id"]] = row.get(
                    "answer_time_level", "0_Unresolved"
                )
        ...
        self.graph.vs["answer_time_level"] = [
            answer_time_levels.get(uid, "0_Unresolved") for uid in unique_users
        ]
