...
class VoteNetworkBuilder(GraphBuilder):
    ...
    def build_vote_network(self, limit: int = 100) -> Tuple[ig.Graph, pd.DataFrame]:
        ...
        df["vote_type"] = df["vote_type_id"].map(
            {1: "accepted", 2: "upvote", 3: "downvote"}
        )
        ...
        if edges:
            ...
            self.graph.es["weight"] = [
                edge_weights[(min(e[0], e[1]), max(e[0], e[1]))] for e in edges
            ]

        vote_counts = (
            df.groupby("post_owner_id")["vote_type"]
            .value_counts()
        ...
        def get_dominant_vote_type(uid):
            votes = user_votes.get(uid, [])
            if not votes:
                return "none"
        ...
        self.graph.vs["vote_type"] = [
            get_dominant_vote_type(uid) for uid in unique_users
        ]
...
class VotingBehaviorAnalyzer:
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vote_type_distribution": self.votes_df["vote_type"]
            .value_counts()
            .to_dict(),
        }
