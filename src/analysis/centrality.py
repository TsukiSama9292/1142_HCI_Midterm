...
class CentralityAnalyzer:
    ...
    def run(self, limit: int = 100) -> Dict[str, Any]:
        ...
        self.graph, self.users_df = self.builder.build_answer_network_with_reputation(limit=limit)
    ...
    def _generate_summary(self, df: pd.DataFrame, correlation: float) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'high_rep_avg_betweenness': high_rep['betweenness'].mean() if len(high_rep) > 0 else 0,
            'low_rep_avg_betweenness': low_rep['betweenness'].mean() if len(low_rep) > 0 else 0,
            'reputation_centrality_correlation': correlation,
